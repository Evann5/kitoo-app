...
test("accueil → chat de soutien → envoi → réponse simulée + ressource détresse", async ({
  ...
  // Réponse rapide (clavier) → intention sommeil.
  const quickReplies = page.getByRole("group", { name: /réponses rapides/i });
  await quickReplies.getByRole("button", { name: /du mal à dormir/i }).click();
  await expect(page.getByText(/sommeil|dormir|nuit/i).first()).toBeVisible();

  // Envoi libre → intention stress (réponse + suggestion exercice).
  ...
  await expect(page.getByText(/respiration/i).first()).toBeVisible();

  // Échange varié : le même message ne renvoie pas deux fois la même réponse.
  await page.getByLabel(/écris ton message/i).fill("je suis stressé");
  await page.getByRole("button", { name: "Envoyer" }).click();
  await expect(page.getByText("je suis stressé")).toHaveCount(2);

  // Message de détresse → ressource d'aide (3114), prioritaire.
  ...
  // Nouvelle conversation : efface le fil, et ça persiste après rechargement.
  await page.getByRole("button", { name: /nouvelle conversation/i }).click();
  await expect(page.getByText("je suis stressé")).toHaveCount(0);
  await page.reload();
  await expect(page.getByText("je suis stressé")).toHaveCount(0);
